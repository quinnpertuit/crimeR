...
library(raster)
gc = as.numeric(geocode("Washington DC")) # centre point of the raster images to grab
x_adjust = 0.0 # move centrepoint in x location
y_adjust = 0.0 # move y direction
gc = gc + c(x_adjust, y_adjust)
ggmap(get_map(location = gc ))
gm = get_map(location = gc, zoom = 12)
ggmap(gm)
gm = get_map(gc, zoom = 12, maptype = "satellite")
ggmap(gm)
...
writeRaster(grm, filename = "dc.tiff", format="GTiff", overwrite = TRUE) # save output
# writeRaster(grm, filename = "~/Dropbox/highbury.bmp", format="BMP", overwrite = TRUE) # save output

z=readShapePoly(system.file("DC Census Tracts.shp", package="maptools")[1])

dsn <- system.file(getwd(), package = "rgdal")[1]
try(ogrInfo(dsn, "Census_Tracts_in_2010"))
ogrInfo("Census_Tracts_in_2010 (1).csv", "Census_Tracts_in_2010 (1).csv")

nhbds <- readOGR("Census_Tracts_in_2010 (1).csv", "Census_Tracts_in_2010 (1).csv") # use rgdal to preserve projection
