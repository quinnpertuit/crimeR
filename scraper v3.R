...
outListDF = rbind(bind_rows(outList1),bind_rows(outList2)) %>% select(-seq_i) %>% unique()

outCrimeDF = outListDF %>%
  ...
  filter(year(rpt_date) %in% c(2010:2019)) %>%
  select(-timename,-timenum) %>% unique() %>%
  filter(rpt_date<=as.Date('2019-06-01')) %>%
  filter(rpt_date>=as.Date('2010-02-01'))


plot1 = outCrimeDF %>%
  filter(offense=="MOTOR VEHICLE THEFT") %>% 
  mutate(ds = floor_date(rpt_date, unit = 'weeks')) %>% 
  group_by(ds) %>% 
  summarise(n=n())

ggplot(plot1) +
  geom_line(aes(x=ds, y=n)) +
